...
- .github/workflows/release.yml: Release 仅发布 EXE 与 zip（内含 EXE），不再附带 BAT
- src/sirius_toolbox/app.py: CLI 参数解析（poi/xhs/webui）、调度与启动
- src/sirius_toolbox/core/runtime_preflight.py: 启动前运行时预检（依赖模块 + Chromium 内核），失败阻断主体启动
...
## 更新日志
- 2026-04-01: 修复 EXE 模式 Chromium 预检路径问题：启动预检阶段为 Playwright 浏览器运行时设置持久目录（`%LOCALAPPDATA%\\ms-playwright`），避免 `_MEI` 临时目录丢失导致 `chromium executable missing`；并增加模块内安装回退路径；同步更新 `README.md`、`CHANGELOG.md` 与 `WEBUI_VERSION`。
- 2026-04-01: 调整 `release.yml` 发布产物，不再生成或发布 `start-webui.bat`，Release 附件改为仅 `SiriusToolbox.exe` 与 zip（内含 EXE）；同步更新 `README.md`、`sirius-toolbox-build` Skill、`CHANGELOG.md` 与 `WEBUI_VERSION`。
- 2026-04-01: 新增 `core/runtime_preflight.py` 并接入 `app.py` 启动链路，EXE/源码启动前统一校验关键依赖与 Chromium 内核，缺失时优先尝试自动修复后再启动主体；同步更新 `README.md`、`CHANGELOG.md` 与 `WEBUI_VERSION`。
